...
function processActionsForUser(user) {
  ...
  // for a user can be quite large. The SQL generated by a BtUser.findOne
...
function doUnblock(sourceBtUser, sinkUid) {
  return Q.ninvoke(twitter, 'blocks', 'destroy', {
    ...
    .spread(function(result, response) {
      logger.trace('/mutes/users/destroy', sourceBtUser, sinkUid,
        response.statusCode);
      return result;
    });
}
  ...
  return Q.all(actions.map(function(action) {
    ...
    return doUnblock(btUser, action.sink_uid).then(function() {
      ...
      if (err && (err.statusCode === 401 || err.statusCode === 403)) {
        ...
        logger.info('Error /mutes/users/destroy 429 for', btUser, 'delaying');
        pauseUntil[btUser.uid] = new Date(new Date().getTime() + 15 * 60 * 1000);
        return Q.resolve(null);
      } else if (err.statusCode) {
        logger.warn('Error /mutes/users/destroy', err.statusCode, btUser,
          '-->', action.sink_uid);
        // Don't change the state of the action: It will be retried later.
        return Q.resolve(null);
      } else {
        logger.error('Error /mutes/users/destroy', err);
        return Q.resolve(null);
      }
    });
  }));
...
function processBlocksForUser(btUser, actions) {
  ...
  return getFriendships(btUser, sinkUids
      ...
      var indexedFriendships = _.keyBy(friendships, 'id_str');
  ...
  if (newState) {
    ...
    return doBlock(sourceBtUser, sink_uid
        ...
        if (err && (err.statusCode === 401 || err.statusCode === 403)) {
          ...
          logger.warn('Error /mutes/users/create', err.statusCode,
