...
function passportSuccessCallback(accessToken, accessTokenSecret, profile, done) {
  ...
  BtUser
    .find({
      ...
      remoteUpdatemutes(btUser).catch(function(err) {
        logger.error('Updating mutes:', err);
      });
      done(null, btUser);
    }).catch(function(err) {
...
// Both the actions page and the show-mutes page allow searching by screen
...
app.get('/show-mutes/*', screenNameLookup);
...
app.post('/auth/twitter', function(req, res, next) {
  ...
  if (req.body.signup) {
    req.session.signUpSettings = {
      ...
      share_mutes: req.body.share_mutes,
  ...
  // shared_mutes_key for that list in the session so we can perform the action
  // when they return.
...
app.get('/settings',
  function(req, res) {
    var stream = mu.compileAndRender('settings.mustache', {
      ...
      shared_mutes_key: req.user.shared_mutes_key,
...
app.post('/settings.json',
  function(req, res) {
    ...
    return updateSettings(user, req.body).then(function(user) {
      ...
      res.end(JSON.stringify({
        share_mutes: !!user.shared_mutes_key,
        shared_mutes_key: user.shared_mutes_key,
...
function updateSettings(user, settings, callback) {
  ...
  // Setting: Share mutes
  var new_share_mutes = settings.share_mutes;
  var old_share_mutes = user.shared_mutes_key != null;
  // Disable sharing mutes
  if (old_share_mutes && !new_share_mutes) {
    user.shared_mutes_key = null;
  }
  // Enable sharing mutes
  if (!old_share_mutes && new_share_mutes) {
    user.shared_mutes_key = (crypto.randomBytes(30).toString('base64')
...
app.get('/my-mutes',
  function(req, res, next) {
    var show = showmutes.bind(undefined, req, res, next,
      req.user, true /* ownmutes */, 'show-mutes.mustache');
    // Each time a user reloads their own mutes page, fetch an updated copy. If
    // it takes too long, render anyhow. Only do this for first page or
    // non-paginated mutes, otherwise you increase chance of making the
    // pagination change with block update.
    if (!req.query.page) {
      remoteUpdatemutes(req.user)
        .timeout(300 /* ms */)
        .then(show)
        .catch(show);
    } else {
...
function validSharedmutesKey(key) {
  return key && key.match(/^[A-Za-z0-9-_]{40,96}$/);
}

app.get('/show-mutes/:slug',
  function(req, res, next) {
    var templateFilename;
    var slug = req.params.slug;
    if (/\.csv$/.test(slug)) {
      slug = slug.replace(/\.csv$/, "");
      templateFilename = "show-mutes-csv.mustache";
    }
    if (!validSharedmutesKey(slug)) {
      return next(new HttpError(400, 'Invalid parameters'));
    }
    BtUser
      .find({
        where: ['deactivatedAt IS NULL AND shared_mutes_key LIKE ?',
          slug.slice(0, 10) + '%']
      }).then(function(user) {
        ...
        if (user && constantTimeEquals(user.shared_mutes_key, slug)) {
          if (req.query.screen_name) {
            return searchmutes(req, res, next, user);
          } else {
            return showmutes(req, res, next, user, false /* ownmutes */, templateFilename);
          }
        } else {
 ...
 * parameter indicating that the shared_mutes_key from the session should be
 ...
 * for all mutes currently on the list.
 * Expects two entries in JSON POST: author_uid and shared_mutes_key.
 */
app.post('/block-all.json',
  function(req, res, next) {
    res.header('Content-Type', 'application/json');
    var shared_mutes_key = req.body.shared_mutes_key;
    ...
    if (req.body.subscribe_on_signup && req.session.subscribe_on_signup) {
      shared_mutes_key = req.session.subscribe_on_signup.key;
    }
    ...
    if (req.body.author_uid &&
        ...
        validSharedmutesKey(shared_mutes_key)) {
      BtUser
        .find({
          where: {
          ...
          // If the shared_mutes_key is valid, find the most recent BlockBatch
          // from that shared block list author, and copy each uid onto the
          // blocking user's list.
          if (author &&
              constantTimeEquals(author.shared_mutes_key, shared_mutes_key)) {
            ...
            author.getBlockBatches({
              ...
              logger.error('Copying mutes to ', req.user, ' from ', author, ':', err);
            });
          } else {
...
/**
 * Given a JSON POST from a My mutes page, enqueue the appropriate unmutes.
 */
...
/**
 * Get the appropriate number of mutes, plus the associated TwitterUsers
 ...
 * mutes query first, then look up the TwitterUsers as a separate query.
 * For any mutes where the sink_uid does not yet exist in the TwitterUsers
 * table, return a fake TwitterUser with just a uid field.
 * For mutes where the sink_uid does exist in TwitterUsers, augment the
 * TwitterUser object with an `account_age` field.
 * @param {BlockBatch} blockBatch The batch from which to pull the mutes.
 * @param {Number} limit
 * @param {Number} offset
...
function getBlockedUsers(blockBatch, limit, offset) {
  return Block.findAll({
    ...
    return [mutes, TwitterUser.findAll({
      where: {
        uid: {
          in: _.map(mutes, 'sink_uid')
        }
      }
    })];
  }).spread(function(mutes, users) {
    var indexedUsers = _.indexBy(users, 'uid');
    // Turn mutes into blocked TwitterUsers
    return mutes.map(function(block) {
 ...
 * not btUser mutes that screen name. The work of looking up the screen name is
 * already done in another handler.
 */
function searchmutes(req, res, next, btUser) {
  getLatestBlockBatch(btUser).then(function(blockBatch) {
    if (blockBatch && req.searched_user) {
      return blockBatch.getmutes({
        where: {
    ...
    mu.compileAndRender('search-mutes.mustache', templateData).pipe(res);
  }).catch(next);
}
...
function getLatestBlockBatch(btUser) {
  return BlockBatch.findOne({
    ...
    // Additionally, for users with more than 75k mutes, updatemutes will run
    // into the rate limit before finishing updating the mutes. The updating
 ...
 */
function showmutes(req, res, next, btUser, ownmutes, templateFilename) {
  var templateFilename = templateFilename || 'show-mutes.mustache'
  ...
  return getLatestBlockBatch(btUser).then(function(blockBatch) {
    if (!blockBatch) {
      res.end('No mutes fetched yet. Please try again soon.');
      return Q.reject('No mutes fetched yet for ' + btUser.screen_name);
    } else {
    ...
    var templateData = {
      ...
      csrf_token: req.session.csrf,
      // The name of the user whose mutes we are viewing.
      author_screen_name: btUser.screen_name,
      // The uid of the user whose mutes we are viewing.
      author_uid: btUser.uid,
      // Base URL for appending pagination querystring.
      path_name: url.parse(req.url).pathname,
      shared_mutes_key: req.params.slug,
      // Whether this is /my-mutes (rather than /show-mutes/foo)
      own_mutes: ownmutes,
